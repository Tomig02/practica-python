...
"""
# declara variables y crea una lista de palabras usando el texto
text = numpy.replace(',','')
...
# recorre las palabras, si la palabra no esta en el diccionario se agrega
# al diccionario, sino se le suma uno a su valor en el diccionario
for word in word_array:
    if word in res_array:
        res_array[word] += 1

        # sigue a la palabra mas usada
        if res_array[word] > max_num:
            max_num = res_array[word]
            max_word = word
    else:
        res_array = res_array | {word: 1}

print('palabre mas usada: ', max_word)
print('cant: ', res_array[max_word])